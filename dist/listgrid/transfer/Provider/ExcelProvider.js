...
function mustOfficeCrypto() {
    if (!officeCrypto) {
        throw new Error('[@rcm/listgrid] password-protected Excel export requires officecrypto-tool. ' +
...
export const ExcelDownload = async (props) => {
    try {
...
export async function saveExcelFile(wb, fileName, password, logOptions) {
    const xlsx = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    let finalBlob;
    if (password) {
        const buffer = toNodeBuffer(xlsx);
