...
export const DataExporter = ({ config, searchForm, fileName, onClose }) => {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(Modal, { size: '5xl', title: title, animation: 'none', closeOnClickOutside: false, closeOnEscape: false, opened: true, onClose: () => {
                    onClose();
                                    ...
                                    handleExport();
                                }, children: t('form.list.dataTransfer.tab.export.button.download') }) })] }) }), processing && (_jsx(Modal, { size: 'lg', title: `${title}`, opened: processing, closeOnClickOutside: false, closeOnEscape: false, onClose: () => {
                    setProcessing(false);
                }, children: _jsx(DataExportProcessor, { url: url, fields: dataFields, searchForm: searchForm, exportFileName: fileName, addedFields: config?.addedFields, overrideFormData: config?.overrideFormData, process: processing, password: password, onProcessed: () => {
