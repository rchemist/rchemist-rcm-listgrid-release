...
export class DataExportService {
    constructor({ searchForm, url, fields, restrictCount, pagePerCount, setExportable, setFailedCount, setProgress, data, setData, setError, editorFields, addedFields, }) {
    ...
    process() {
        ...
        let data = [];
        const fieldMap = createFieldMap(...this.fields);
        let timeoutId = null;
        const exportData = async () => {
            ...
            if (this.searchForm && this.url) {
                if (!this.total) {
                    ...
                    const pageResult = await PageResult.fetchListData(this.url, this.searchForm).catch((reason) => {
                ...
                if (currentProgress >= 100) {
                    ...
                    // 실제 데이터 페치 부분
                    await Promise.all(pageResult.list.map(async (item) => {
                        const row = [];
                        await Promise.all(Array.from(fieldMap.entries()).map(async ([key, value]) => {
                        ...
                        if (this.addedFields !== undefined) {
                            ...
                            if (addedFields) {
                                await Promise.all(addedFields.map(async (column) => {
                                    row.push(column);
                                }));
                            }
                        }
