...
export function subStringBeforeLast(value, splitCode) {
...
export function subStringBefore(value, splitCode) {
    if (!value.includes(splitCode)) {
        return value;
    }
    const split = value.split(splitCode);
...
export function subStringAfterLast(value, splitCode) {
    if (!value.includes(splitCode)) {
        return value;
    }
    const split = value.split(splitCode);
...
export function subStringAfter(value, splitCode) {
    if (!value.includes(splitCode)) {
        return value;
    }
    const split = value.split(splitCode);
...
export function subStringBetween(value, startCode, endCode) {
    return subStringBefore(subStringAfter(value, startCode), endCode);
}
export function subStringBetweenLast(value, startCode, endCode) {
    return subStringBeforeLast(subStringAfterLast(value, startCode), endCode);
}
export function defaultString(value, defaultValue = '') {
    if (isBlank(value)) {
        return defaultValue;
    }
    else {
...
export function appendPrefix(str, prefix, split) {
    if (!isBlank(split)) {
        prefix = prefix + split;
    }
...
export function appendSuffix(str, suffix, split) {
    if (!isBlank(split)) {
        suffix = split + suffix;
    }
...
export function removePrefix(str, prefix, split) {
    if (!isBlank(split)) {
        prefix = prefix + split;
    }
    if (!str.startsWith(prefix)) {
...
export function removeSuffix(str, suffix, split) {
    if (!isBlank(split)) {
        suffix = split + suffix;
    }
    if (!str.endsWith(suffix)) {
...
export function appendPrefixSuffix(str, prefix, suffix, split) {
    return appendSuffix(appendPrefix(str, prefix, split), suffix, split);
}
export function getHashCode(data) {
...
export function startsWith(value, prefix, ignoreCase) {
...
export function endsWith(value, postFix, ignoreCase) {
...
export function equalsIgnoreCase(value, other, trim) {
    if (isTrue(trim)) {
        value = value.trim();
        other = other.trim();
    }
 ...
 * @param word
 */
export function addKoreanWordPostfix(type, word) {
...
export function equalsIgnoreBlank(value, other) {
