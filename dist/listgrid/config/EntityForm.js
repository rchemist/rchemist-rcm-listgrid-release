...
export class EntityForm extends EntityFormExtensions {
    ...
    cloneWithEntityForm(entityForm, includeValue) {
        ...
        entityForm.attributes = this.attributes ? new Map(this.attributes) : undefined;
    ...
    setFetchedValue(fieldName, value) {
    ...
    withAppendAdvancedSearchFields(...fields) {
        ...
        if (!list) {
            ...
            if (fetchedEntity) {
                entityForm.fields.forEach((field, key) => {
                    ...
                    if (field.value?.fetched !== undefined)
                        return;
                    if (key.includes('.')) {
                        const keyParts = key.split('.');
                        let objectValue = fetchedEntity;
    ...
    withOverrideSubmitData(fn) {
        this.overrideSubmitData = fn;
        return this;
    }
    async setFetchedValues(entity) {
    ...
    async fetchData(fetchUrl = this.getFetchUrl()) {
    ...
    async internalSave(session, skipValidation, forceIncludeExceptOnSave) {
        // 아래는 기본 저장 로직이다.
                            ...
                            if (errorObject.fieldError) {
                                if (errorObject.fieldError instanceof Map) {
                                    ...
                                    if (errorObject.fieldError.size > 0) {
                                        errorObject.fieldError.forEach((fieldError, fieldName) => {
    ...
    async save(session, skipValidation, forceIncludeExceptOnSave) {
        // 저장 로직이 오버라이드 됐다면
        const actionType = this.getRenderType();
        if (this.onSave) {
            const form = this.clone(true);
            const result = await this.onSave(form);
    ...
    async getSubmitFormData(forceIncludeExceptOnSave) {
        ...
        try {
            ...
            const processManyToOneField = (field, value, targetObject, fieldNameOverride) => {
    ...
    withCheckDuplicate(fieldName, checkDuplicate) {
        const field = this.getField(fieldName);
