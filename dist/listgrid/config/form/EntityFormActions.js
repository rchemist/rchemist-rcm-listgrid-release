...
import { AbstractManyToOneField, FormField, ListableFormField, OptionalField, } from '../../components/fields/abstract';
import { StatusCreatedAtFieldPreset, StatusCreatedAndUpdatedAtFieldPreset, ActiveField, } from '../../components/fields/Preset';
import { DEFAULT_TAB_INFO, DEFAULT_FIELD_GROUP_INFO, STATUS_TAB_INFO, } from '../../config/Config';
...
export class EntityFormActions extends EntityFormData {
    constructor(name, url) {
        super(name, url);
    }
    getListableFieldOrder(field) {
     ...
     */
    clearAlertMessages(includePersistent = false) {
    ...
    withExcludeListFields(...excludeListFields) {
        if (this.excludeListFields) {
            // 중복 제거 후 추가
            const fieldsSet = new Set([...this.excludeListFields, ...excludeListFields]);
    ...
    addFields(props) {
        ...
        for (const field of props.items) {
            ...
            if (target instanceof FormField) {
                ...
                this.fields.set(field.name, target);
            }
     ...
     * @param props
     */
    addCollections(props) {
        return this.addFields(props);
    }
    useListFields(...fieldNames) {
    ...
    getViewOrder(tabId, fieldGroupId, fieldOrder) {
    ...
    getDataFields(fieldNames, dataTransferRules) {
        const dataFields = [];
    ...
    async getDataFieldsFromFields() {
        ...
        for (const field of fields) {
            ...
            const dataField = DataField.create({
                name: field.getName(),
                label: field.label !== undefined && typeof field.label === 'string'
                    ? field.label
                    : field.getName(),
                type: field.type,
    ...
    withFilterable(fieldName, filterable = true) {
        const field = this.getField(fieldName);
        if (field !== undefined && field instanceof ListableFormField) {
    ...
    withNeverDelete(neverDelete = true) {
        ...
        if (neverDelete) {
            ...
            this.withOnInitialize(async (entityForm) => {
    ...
    withCreateStep(createStep) {
        this.setCreateStep(createStep);
        return this;
    }
}
//# sourceMappingURL=EntityFormActions.js.map