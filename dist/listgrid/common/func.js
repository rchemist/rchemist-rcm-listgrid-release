...
export function getWindowPositionClassName(position) {
    // use tailwindCss
    switch (position) {
        case 'center':
...
export function getBgColor(color) {
    ...
    if (AdditionalColorTypes.includes(color)) {
        return getAdditionalColorClass(color);
    }
    const colorValue = color.startsWith('#') ? color.slice(1) : color;
    return `bg-[#${colorValue}]`;
}
export function getColorSuffix(color) {
    return color.startsWith('#') ? `[${color}]` : color;
}
export function getMaxWidthClassName(size) {
    switch (size) {
        case 'xs':
            return 'max-w-xs';
        case 'sm':
...
export function getGutterValue(gutter) {
...
export function getResponsiveClasses(value, prefix, valueMap) {
    const getValue = (val) => {
    ...
    if (typeof value === 'object' && value !== null) {
        const classes = [];
        if (value.base)
            classes.push(`${prefix}-${getValue(value.base)}`);
        if (value.xs)
            classes.push(`sm:${prefix}-${getValue(value.xs)}`);
        if (value.sm)
            classes.push(`sm:${prefix}-${getValue(value.sm)}`);
        if (value.md)
            classes.push(`md:${prefix}-${getValue(value.md)}`);
        if (value.lg)
            classes.push(`lg:${prefix}-${getValue(value.lg)}`);
        if (value.xl)
            classes.push(`xl:${prefix}-${getValue(value.xl)}`);
        return classes.join(' ');
    }
    return `${prefix}-${getValue(value)}`;
}
export function getShapeClassName(shape) {
    switch (shape) {
        case 'pill':
            return 'rounded-full';
        case 'box':
...
export function getColorClass(color, variant) {
