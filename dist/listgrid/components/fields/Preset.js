...
export const ExternalIdField = (props) => {
    ...
    return new StringField('externalId', order)
        ...
        .withCheckButtonValidation(checkDuplicateValueProcess({
        url: props.fetchUrl,
...
export const StatusDateAndRevisionPreset = (useUpdatedAt = true, useRevision = true) => {
    ...
    if (useUpdatedAt) {
        items.push(new DatetimeField('updatedAt', 100100)
            .withLabel('수정일')
            .withViewPreset(MODIFY_ONLY)
            .withReadOnly(true));
    }
...
export const DiscardedPublishStatusTypes = [{ label: '폐기', value: 'DISCARDED' }];
            ...
            if (publishStatus && publishStatus instanceof SelectField) {
                if (publishStatusValue === 'PUBLISHED') {
                    publishStatus
                        ...
                        .withHelpText('현재 사용중인 상태 입니다. 게시 상태를 `폐기`로 변경하면 더 이상 사용할 수 없습니다.');
