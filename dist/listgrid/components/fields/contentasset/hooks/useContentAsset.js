...
export const useContentAsset = ({ value: initialValue, onChange, onError, clearError, entityForm, maxItems, readonly = false }) => {
    ...
    // 자산 업데이트
    const handleUpdateAsset = useCallback((index, field, value) => {
    ...
    // 제목 필드 blur 시 중복 검증
    const handleTitleBlur = useCallback((index, value) => {
    ...
    // 파일 업로드 핸들러
    const handleFileUpload = useCallback(async (index, file, onUploadProgress) => {
        setLoading(true);
        try {
