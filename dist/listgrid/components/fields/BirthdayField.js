...
const BirthdayInput = ({ name, value, onChange, readonly = false, hasError = false, placeholder = 'YYYY-MM-DD', includeHyphen = false }) => {
    ...
    const getReturnValue = (formatted) => {
        if (includeHyphen) {
            return formatted;
        }
 ...
 */
export class BirthdayField extends ListableFormField {
    constructor(name, order, includeHyphen = false) {
    ...
    renderInstance(params) {
        return (async () => {
            ...
            return (_jsx(BirthdayInput, { name: inputParams.name, value: inputParams.value ?? '', onChange: (value, commit) => {
                    inputParams.onChange?.(value, commit);
                }, readonly: inputParams.readonly, placeholder: inputParams.placeHolder, includeHyphen: this.includeHyphen }));
        })();
    }
    /**
     * 목록에 표시될 때의 렌더링 로직
     ...
     */
    withIncludeHyphen(includeHyphen) {
        this.includeHyphen = includeHyphen;
        return this;
    }
    static create(props) {
