...
function parseFilterSegment(segment) {
    ...
    if (parts.length >= 3) {
        ...
        const queryConditionType = OPERATOR_MAP[opKey];
        if (queryConditionType) {
            ...
            if ((queryConditionType === 'IN' || queryConditionType === 'NOT_IN') &&
                rawValue.includes('|')) {
 ...
 */
export const parseAsQuickSearch = createParser({
    parse: (value) => {
        if (!value || value.trim() === '')
            return null;
        return value;
 ...
 */
export function hasUrlParams(urlState) {
    return (urlState.page !== null ||
        urlState.pageSize !== null ||
        urlState.q !== null ||
        urlState.sort !== null ||
        urlState.filters !== null);
}
//# sourceMappingURL=urlStateParsers.js.map