...
export const useListGridLogic = (props) => {
    ...
    const enableHandleData = manyToOne === undefined && !isTrue(props.options?.readonly);
    const isSubCollection = props.options?.subCollection !== undefined;
    const isPopup = !!props.options?.popup;
    ...
    const showMessages = useCallback((error, messages) => {
        if (error) {
    ...
    const fetchData = useCallback((form) => {
        setLoading(true);
        (async () => {
            ...
            const hasClientExtensions = entityForm.hasClientExtensions &&
                entityForm.hasClientExtensions(ExtensionPoint.PRE_FETCH_LIST, ExtensionPoint.POST_FETCH_LIST);
            ...
            listGrid.fetchData(finalSearchForm, {
                ...
                if (!isEmpty(result.errors)) {
                    ...
                    if (originalQuickSearchFields.length > 0) {
                        newSearchForm.handleQuickSearch(finalSearchForm.getQuickSearchValue() ?? '', originalQuickSearchFields);
                    }
                    ...
                    const quickSearchValue = getQuickSearchFromSearchForm(newSearchForm, quickSearchProperty?.name, quickSearchProperty?.orFields);
                    if (quickSearchValue) {
    ...
    const initializeSearchForm = useCallback(async () => {
        ...
        if (urlStateHook.isEnabled) {
            if (urlStateHook.hasUrlParams) {
                ...
                const quickSearchValue = getQuickSearchFromSearchForm(searchForm, quickSearchProperty?.name, quickSearchProperty?.orFields);
                if (quickSearchValue) {
                    setSearch(quickSearchValue);
                }
                return searchForm;
        ...
        let searchForm = isMainEntity
            ? await getSessionStorageObject(hashKey, (value) => SearchForm.deserialize(value))
            : undefined;
            ...
            (async () => {
                await showConfirm({
                    ...
                    onConfirm: async () => {
                        ...
                        const result = props.options?.delete?.onDelete !== undefined
    ...
    useEffect(() => {
        ...
        if (!urlStateHook.isEnabled || !searchForm)
            return;
    ...
    const onSelect = (props.options?.onSelect ??
        ...
        (isSubCollection
            ...
            : undefined));
    const performQuickSearch = (search) => {
    ...
    const sortRowsPriority = async (changed) => {
     ...
     */
    const getCleanSearchForm = useCallback(async () => {
        const originalSearchForm = listGrid.getSearchForm();
        let searchForm = originalSearchForm.clone();
        const pageSize = props.options?.defaultPageSize ?? getGlobalPageSize();
        searchForm.withPageSize(pageSize);
...
//# sourceMappingURL=useListGridLogic.js.map