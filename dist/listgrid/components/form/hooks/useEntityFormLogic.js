 ...
 */
export function useEntityFormLogic(props) {
    // 상태 선언
    ...
    const [headerAreaContent, setHeaderAreaContent] = useState(null);
    ...
    const clearAutoSaveRef = useRef(() => { });
    const postSave = useCallback(async (entityForm) => {
    ...
    const openBaseLoadingCallback = useCallback((open) => {
        setOpenBaseLoading(open);
    }, [setOpenBaseLoading]);
    // 모달 관련 함수들
    const { openModal, closeModal, closeTopModal, findModal, updateModalData } = useModalManagerStore();
    const showModal = useCallback((options) => {
        return openModal(options);
    }, [openModal]);
    const handleCloseModal = useCallback(async (id) => {
        await closeModal(id);
    }, [closeModal]);
    ...
    const handleUpdateModalData = useCallback((id, data) => {
    ...
    const changeCurrentStep = (stepNumber) => {
        (async () => {
    ...
    const resetEntityForm = useCallback(async (delay, preserveState = true) => {
        // 현재 상태 보존
...
//# sourceMappingURL=useEntityFormLogic.js.map