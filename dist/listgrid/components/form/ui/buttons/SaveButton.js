...
export const SaveButton = ({ entityForm, postSave, router, pathname, setEntityForm, buttonLinks, openBaseLoading, session, buttonClassNames, ...props }) => {
    ...
    return (_jsx("button", { type: "button", disabled: isSaving, className: cn("btn btn-primary text-[13px] disabled:opacity-50 disabled:cursor-not-allowed", buttonClassNames?.save ?? classNames.buttons?.save), onClick: (e) => {
            e.stopPropagation();
            if (isSaving)
                return;
            (async () => {
